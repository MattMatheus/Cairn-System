# memory-cli Product Vision (Long-Term)
...
## Core Insight
Agents spend significant context capacity on orientation, and that cost repeats every session. memory-cli externalizes operating knowledge into a governed memory layer so agents and humans can access the right context quickly and consistently.
