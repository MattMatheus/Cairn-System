...
The goal is to preserve that value while stripping away the surrounding runtime posture that does not fit the personal Athena system.

## Product Boundary

Athena Code Graph V0 should expose a thin local contract around a repository-analysis backend.
...
- `athena-use`: advertises code-graph capability and explains when it fits
- `athena-work`: decides when repo analysis is appropriate in a workflow
- `athena-mind`: stores distilled conclusions, not raw graph state
...
- registry and guidance home: `products/athena-use`
- implementation posture: a thin Athena-facing wrapper or contract around GitNexus, not a new graph engine
...
- results should be compact enough to complement AthenaMind rather than overwhelm it
...
PM recommendation is to keep GitNexus as a candidate backend engine while stripping the exposed Athena contract to the smallest useful surface.
...
- agents can discover and inspect the capability through AthenaUse without inheriting full MCP noise
- important findings still end up in AthenaMind as durable conclusions

## Follow-On Note

Firecrawl already triggered the stronger conclusion:

- build a smaller native Athena intake tool instead of integrating the full external product
