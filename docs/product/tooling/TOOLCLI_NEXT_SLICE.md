# tool-cli Next Slice Recommendation

## Decision

Implement stronger tool-cli context/schema output next.

Do not start the formal cross-platform tool interface spec as the next implementation slice.

## Why

- The current tool-cli v1 design already defines `context` as the stage-launch integration surface.
...
- improve `tool-cli context` to emit stronger schema-oriented fields
...
- `docs/decisions/ADR-0007-toolcli-next-slice-context-schema-before-formal-interface.md`
