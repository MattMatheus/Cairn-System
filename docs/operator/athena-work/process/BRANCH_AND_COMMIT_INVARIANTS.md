...
description: Non-negotiable AthenaWork rules for branch discipline, state movement, observer closure, and single-commit cycles.
doc_type: claim
status: draft
domain: process
related:
...
AthenaWork cycle execution depends on a few invariants: work happens on the required branch, state moves through canonical lanes, observer runs before commit, and each completed cycle produces exactly one commit.
...
These rules keep AthenaWork observable and auditable. They ensure that a cycle artifact set, observer report, and final commit describe the same unit of work.
