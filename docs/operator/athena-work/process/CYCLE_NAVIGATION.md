...
description: Minimal route through an AthenaWork delivery cycle, from branch check to stage launch to observer closure.
doc_type: procedure
status: draft
domain: process
related:
...
An AthenaWork cycle starts with branch safety, moves through a stage launcher and backlog state transition, and ends only after observer output plus a single cycle commit.
...
1. Confirm the active branch matches `ATHENA_REQUIRED_BRANCH` (default `dev`).
2. Launch the needed stage with `products/athena-work/tools/launch_stage.sh <stage>`.
...
6. Close the cycle with `products/athena-work/tools/run_observer_cycle.sh --cycle-id <cycle-id>`.
