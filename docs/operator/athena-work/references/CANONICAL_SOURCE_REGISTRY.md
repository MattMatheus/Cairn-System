...
description: Specification for recording official vendor docs, allowed domains, and version assumptions so AthenaWork agents use the right external sources.
doc_type: reference
status: draft
---
...
AthenaWork needs explicit records for official external docs. A canonical source record tells an agent which vendor docs to trust, which version to assume, and when to consult those docs instead of guessing or using stale internal notes.

## Intended Audience

- maintainers authoring AthenaWork operator/reference content
...
This lets AthenaWork combine internal usage guidance with vendor truth.
...
```yaml
title: Terraform 1.9
description: Official Terraform docs for AthenaWork tasks that touch Terraform CLI or HCL semantics.
...
avoid_when:
  - the task is only about AthenaWork process or local conventions
notes:
...
- `docs/operator/athena-work/references/canonical-sources/`
...
- `AthenaWork infra changes depend on [[terraform-1-9]].`
