...
- `tool-cli_<version>_darwin_arm64.zip`
- `tool-cli_<version>_windows_amd64.zip`
- `SHA256SUMS`
...
Artifacts are written under `.cairn/artifacts/releases/<version>/`.
...
2. work harness doc tests
3. `go test ./...` in `products/tool-cli`
4. `go test ./...` in `products/memory-cli`
5. `./tools/dev/build_release_artifacts.sh --version "azure-<build-id>"`

The resulting zipped binaries and `SHA256SUMS` file are published as the `cairn-release-artifacts` build artifact.
