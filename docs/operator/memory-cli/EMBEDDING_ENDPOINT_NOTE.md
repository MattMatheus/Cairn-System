...
memory-cli currently works with an Ollama-compatible base endpoint.

Validated host pattern:

- base endpoint: `http://192.168.1.35:11434`
- API path used by memory-cli: `/api/embeddings`
...
memory-cli now supports an Ollama model override through:

- `CAIRN_OLLAMA_EMBED_MODEL`
...
For memory-cli, pass the base host URL as `--embedding-endpoint`.
...
export CAIRN_MEMORY_ROOT="${CAIRN_MEMORY_ROOT:-$CAIRN_HOME/memory/default}"
mkdir -p "$CAIRN_MEMORY_ROOT"

(cd products/memory-cli && CAIRN_OLLAMA_EMBED_MODEL="mxbai-embed-large:latest" go run ./cmd/memory-cli verify health \
...
memory-cli applies a latency degradation policy during retrieval.
