...
All must pass:
1. Intake validation passes (`./products/work-harness/tools/validate_intake_items.sh`).
2. Active queue is ranked and explicit in `products/work-harness/delivery-backlog/engineering/active/README.md`.
3. Product-first backlog weighting is applied per `docs/operator/work-harness/process/BACKLOG_WEIGHTING_POLICY.md`.
4. Active stories include traceability metadata (`idea_id`, `phase`, `adr_refs`, metric).
5. Program-state references are updated per `docs/operator/work-harness/process/PROGRAM_OPERATING_SYSTEM.md`.
...
3. `./products/work-harness/tools/run_doc_tests.sh` and story-specific tests pass.
4. Go tests pass locally for each touched Go module (for example `cd products/tool-cli && go test ./...`) and are eligible for CI enforcement in Azure DevOps.
...
5. QA result artifact exists in `products/work-harness/delivery-backlog/engineering/done/QA-RESULT-<story>.md`.
...
3. Required sync targets were updated when workflow behavior changed (`HUMANS.md`, `products/work-harness/AGENTS.md`, `docs/operator/work-harness/DEVELOPMENT_CYCLE.md`, stage prompts, and gates as needed).
...
1. Release bundle exists at `products/work-harness/operating-system/handoff/RELEASE_BUNDLE_<release-id>.md`.
...
Reference template: `products/work-harness/operating-system/handoff/RELEASE_BUNDLE_TEMPLATE.md`.
