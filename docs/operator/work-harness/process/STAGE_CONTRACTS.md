...
description: Short contract per stage describing the outputs and gates required before work harness can hand off or transition state.
doc_type: claim
status: draft
domain: process
related:
...
Each work harness stage has a contract: required outputs, required evidence, and an allowed state transition. A stage is not done because time was spent; it is done because its contract is satisfied.
