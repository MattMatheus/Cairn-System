...
description: Decision rules for choosing the minimum correct verification set before work harness work is handed off or closed.
doc_type: procedure
status: draft
domain: process
canonical_sources:
...
work harness verification is risk-based but never optional. Run the narrowest set of checks that can credibly prove the touched behavior, then include those commands and results in the handoff.
...
2. Run `products/work-harness/tools/run_doc_tests.sh` when docs, prompts, or process behavior changed.
