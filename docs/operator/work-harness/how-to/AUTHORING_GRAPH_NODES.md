...
description: How to write work harness knowledge-base nodes so agents can scan, route, and compose them without loading everything.
doc_type: how_to
status: draft
---
...
- people adding or refactoring work harness docs
...
If the page tells an agent how work harness uses a tool, attach a canonical source record for the tool. Keep the node focused on work harness-specific usage and let the source record point to the vendor docs.

Good pattern:

- node says how work harness uses Podman locally
- source record says which Podman docs and version policy to trust

Bad pattern:

- node pastes large excerpts from Podman docs into work harness docs
