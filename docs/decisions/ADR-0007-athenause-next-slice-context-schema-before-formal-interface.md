# ADR-0007: AthenaUse Next Slice Prioritizes Context Schema Before Formal Interface Spec

Status: Accepted

## Context

AthenaUse v1 is now live enough to validate three things:
...
- ADR-0005 limits AthenaUse v1 to discovery, listing, context, and validation
- ADR-0006 keeps the implementation intentionally narrow and OpenTelemetry-aligned
- `docs/product/tooling/ATHENAUSE_V1.md` explicitly defers full JSON Schema support and execution/runtime expansion
- `docs/product/tooling/MODEL_TOOL_INTERFACE_PREP.md` exists as preparation material, not an approved implementation contract

## Decision

The next approved AthenaUse implementation slice is:

- bounded context/schema shaping for `use-cli context`
...
- keeps work inside the already-approved AthenaUse v1 boundary
...
- enrich `use-cli context` output with schema-oriented fields for required parameters
- tighten tests around emitted context shape
- update AthenaWork stage-launch consumption in the same cycle if needed

The bounded next slice may not:

- introduce `use-cli call`
...
- align `docs/product/tooling/ATHENAUSE_V1.md` with the selected next slice
