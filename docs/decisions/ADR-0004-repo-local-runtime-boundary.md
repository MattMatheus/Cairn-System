...
The `.athena/` directory is the default location for:

- AthenaWork runtime state
...
The `.athena/` directory must be excluded from version control by default.

## Consequences

Positive:

- each repository gets an isolated Athena operating area
...
1. Use `.athena/artifacts/` as the default bootstrap destination for fetched Azure-delivered assets.
