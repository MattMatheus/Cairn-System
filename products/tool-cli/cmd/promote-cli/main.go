...
func repoRoot() string {
	...
	dir := wd
	for {
		candidate := filepath.Join(dir, "products", "memory-cli", "cmd", "memory-cli")
...
func defaultMemoryRoot() string {
	if value := strings.TrimSpace(os.Getenv("CAIRN_MEMORY_ROOT")); value != "" {
		return value
	}
	return filepath.Join(repoRoot(), ".cairn", "memory", "default")
}

func memoryCLIPath() string {
	return filepath.Join(repoRoot(), "products", "memory-cli", "cmd", "memory-cli")
}
