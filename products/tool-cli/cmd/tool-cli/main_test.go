...
tools:
  - id: obsidian.open_vault
    ...
    description: Opens the Cairn vault in Obsidian
...
`
	...
	if strings.Contains(defaultContext, "cairn.intake.normalize_source") {
	...
	if !strings.Contains(plannedContext, "cairn.intake.normalize_source") {
...
func TestApprovedRegistryIncludesActiveIntakeToolInScopedQueries(t *testing.T) {
	...
	if !strings.Contains(out, "cairn.intake.normalize_source") {
...
func TestApprovedRegistryIncludesIntakeInspectToolInScopedQueries(t *testing.T) {
	...
	if !strings.Contains(out, "cairn.intake.inspect_source") {
...
tools:
  - id: gitnexus.repo_graph
    ...
    complements: [memory-cli, obsidian]
    credential: ""
    call:
      type: exec
...
`
	...
	if !strings.Contains(out, "Complements: memory-cli, obsidian") {
