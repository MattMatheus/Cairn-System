...
emit_tool_context() {
  local stage_name="$1"
  local use_product_root="$platform_root/products/athena-use"
  local output=""
  local err=0
  ...
  elif [ -f "$use_product_root/go.mod" ] && [ -f "$use_product_root/cmd/use-cli/main.go" ]; then
    if ! output="$(cd "$use_product_root" && go run ./cmd/use-cli context --stage "$stage_name" 2>&1)"; then
      err=$?
    fi
  elif command -v use-cli >/dev/null 2>&1; then
    if ! output="$(use-cli context --stage "$stage_name" 2>&1)"; then
      err=$?
    fi
  else
    echo "warning: tool context skipped; use-cli unavailable" >&2
    return 0
  fi

  if [ "$err" -ne 0 ]; then
    echo "warning: tool context skipped; use-cli failed: $output" >&2
    return 0
  fi
...
EOF
    ...
    ;;
  *)
    echo "usage: products/athena-work/tools/launch_stage.sh [engineering|qa|pm|planning|architect|cycle]" >&2
    exit 1
    ;;
esac
