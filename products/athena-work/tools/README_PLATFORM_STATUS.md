...
The AthenaWork tools folder currently contains a mix of:
...
These are the AthenaWork-owned scripts that still matter for the internal-beta path.
When in doubt, prefer the root platform checks in `tools/platform/` first, then use AthenaWork product tools for stage flow and observer behavior.
