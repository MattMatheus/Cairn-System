...
Navigation and operating guide for AthenaWork state-harness agents.

## Mission Context
AthenaWork is a state-harness product with a strict staged delivery workflow.

## Repository Context
Primary product boundary: `products/athena-work/`.
Canonical workspace boundary: `workspace/`.
Local runtime state belongs under `.athena/`.

## First 5 Minutes
1. Read `products/athena-work/HUMANS.md`.
2. Read `products/athena-work/DEVELOPMENT_CYCLE.md`.
3. Read `workspace/AGENTS.md`.
4. Read `products/athena-work/delivery-backlog/engineering/active/README.md`.
5. Launch requested stage with `products/athena-work/tools/launch_stage.sh <stage>`.
...
- Branch must match `ATHENA_REQUIRED_BRANCH` (default `dev`).
...
- Run observer after each completed cycle: `products/athena-work/tools/run_observer_cycle.sh --cycle-id <cycle-id>`.
...
5. related `products/athena-work/operating-system-vault/*` contracts when behavior actually changes there

## Notes
- Prefer current platform paths over any historical repo-era references that still appear in imported docs.
- The active operator surface is `products/athena-work/`, `workspace/`, and `.athena/`.
