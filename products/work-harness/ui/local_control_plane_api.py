...
DEFAULT_STATE_FILE = Path("/workspace/products/work-harness/operating-system/state/backend_read_model_v1.json")
RUNTIME_STATE_FILE = Path("/workspace/products/work-harness/operating-system/state/runtime/backend_read_model_v1.local.json")
MIGRATION_LOG_FILE = Path("/workspace/products/work-harness/operating-system/state/runtime/migration_events_v1.local.json")
...
CORE_DOCS_INDEX = [
    ...
    },
    {
        ...
        "path": "docs/operator/work-harness/operations/LOCAL_CONTROL_PLANE_QUICKSTART.md",
    },
]
INGEST_TOOL = "/workspace/products/work-harness/tools/ingest_artifact_bundle.sh"
LAUNCH_PACKAGE_TOOL = "/workspace/products/work-harness/tools/generate_launch_authorization_package.sh"
LAUNCH_VALIDATE_TOOL = "/workspace/products/work-harness/tools/validate_launch_authorization_package.sh"
LAUNCH_PACKAGE_DIR = Path("/workspace/products/work-harness/operating-system/handoff")
...
def load_state():
    runtime_path = Path(os.getenv("CAIRN_RUNTIME_STATE_FILE", str(RUNTIME_STATE_FILE)))
...
def _resolve_story_path(story_id):
    candidates = [
        Path(f"products/work-harness/delivery-backlog/engineering/active/{story_id}.md"),
        Path(f"products/work-harness/delivery-backlog/engineering/qa/{story_id}.md"),
        Path(f"products/work-harness/delivery-backlog/engineering/done/{story_id}.md"),
    ]
    ...
    docs.append(
        {
            ...
            "path": "products/work-harness/delivery-backlog/engineering/active/README.md",
        }
    )
...
def build_board():
    ...
    mode = os.getenv("CAIRN_UI_ACCESSIBILITY_MODE", "normal").strip().lower()
    if mode not in {"normal", "low-vision"}:
        mode = "normal"
    theme = os.getenv("CAIRN_UI_THEME", "default").strip().lower() or "default"
...
def migration_log_path():
    return Path(os.getenv("CAIRN_MIGRATION_LOG_FILE", str(MIGRATION_LOG_FILE)))
...
def model_chat_completion(prompt):
    ...
    timeout_s = 20
    try:
        timeout_s = int(os.getenv("CAIRN_MODEL_TIMEOUT_S", "20"))
    except Exception:  # noqa: BLE001
        timeout_s = 20
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_artifact_ingest(self):
        ...
        with tempfile.TemporaryDirectory(prefix="cairn-ingest-") as tmp_dir:
    ...
    def do_GET(self):
        ...
        self.send_json(
            {
                "service": "api",
                "message": "work harness local control-plane bootstrap",
            }
        )
