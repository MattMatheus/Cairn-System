...
base_ref="${CAIRN_SECURITY_GATE_BASE_REF:-origin/main}"
...
required_vars=(
  ...
  CAIRN_SECURITY_NONCE_ISSUED_AT_UTC
...
if [[ ! "${CAIRN_SECURITY_CHANGE_NONCE}" =~ ^[A-Z0-9]{12,64}$ ]]; then
  echo "FAIL: CAIRN_SECURITY_CHANGE_NONCE must match [A-Z0-9]{12,64}"
  exit 1
fi

if [[ "${CAIRN_SECURITY_CHANGE_ACK}" != "LAUNCH AUTHORIZED" ]]; then
  echo "FAIL: CAIRN_SECURITY_CHANGE_ACK must equal 'LAUNCH AUTHORIZED'"
  exit 1
fi

window_minutes="${CAIRN_SECURITY_GATE_WINDOW_MINUTES:-15}"
if [[ ! "$window_minutes" =~ ^[0-9]+$ || "$window_minutes" -le 0 ]]; then
  echo "FAIL: CAIRN_SECURITY_GATE_WINDOW_MINUTES must be a positive integer"
  exit 1
fi

age_minutes="$(
  python3 - "$CAIRN_SECURITY_NONCE_ISSUED_AT_UTC" <<'PY'
...
if [[ ! "$age_minutes" =~ ^-?[0-9]+$ ]]; then
  echo "FAIL: unable to parse CAIRN_SECURITY_NONCE_ISSUED_AT_UTC"
  exit 1
fi
