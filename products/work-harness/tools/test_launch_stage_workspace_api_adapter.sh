...
launch_script="$platform_root/products/work-harness/tools/launch_stage.sh"
current_branch="$(git -C "$platform_root" branch --show-current)"
if [[ -z "$current_branch" || "$current_branch" == "HEAD" ]]; then
  current_branch="${BUILD_SOURCEBRANCHNAME:-main}"
fi

output="$(WORKSPACE_API_ENABLED=true WORKSPACE_API_CURL_BIN=missing-curl CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" pm)"
...
set +e
CAIRN_DIRECTION_CHANGE=true CAIRN_DIRECTION_AUDIT_LOG_PATH="$direction_log" CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" pm >/dev/null 2>"$err_file"
code=$?
set -e
...
confirm_output="$(CAIRN_DIRECTION_CHANGE=true CAIRN_DIRECTION_CONFIRMATION_ID=CONFIRM-TEST-1 CAIRN_DIRECTION_CONFIRMED_BY='Matt' CAIRN_DIRECTION_CONFIRMED_AT='2026-02-27T00:00:00Z' CAIRN_DIRECTION_SCOPE='qa-stage' CAIRN_DIRECTION_EXPIRY='2099-12-31T23:59:59Z' CAIRN_DIRECTION_AUDIT_LOG_PATH="$direction_log" CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" qa)"
...
set +e
CAIRN_DIRECTION_CHANGE=true CAIRN_DIRECTION_CONFIRMATION_ID=CONFIRM-TEST-OLD CAIRN_DIRECTION_CONFIRMED_BY='Matt' CAIRN_DIRECTION_CONFIRMED_AT='2026-02-27T00:00:00Z' CAIRN_DIRECTION_SCOPE='pm-stage' CAIRN_DIRECTION_EXPIRY='2000-01-01T00:00:00Z' CAIRN_DIRECTION_AUDIT_LOG_PATH="$direction_log" CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" pm >/dev/null 2>"$err_file"
code=$?
set -e
...
set +e
CAIRN_UNDOCUMENTED_AGENT_COMM=true CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" pm >/dev/null 2>"$err_file"
code=$?
set -e
...
research_output="$(CAIRN_REQUIRED_BRANCH="$current_branch" \
  CAIRN_UNDOCUMENTED_AGENT_COMM=true \
  CAIRN_LANE_CONTEXT=research \
  CAIRN_RESEARCH_COMM_EXCEPTION=true \
  CAIRN_SOURCE_AGENT=agent-alpha \
  CAIRN_TARGET_AGENT=agent-beta \
  CAIRN_REASON='investigate parser behavior' \
  CAIRN_CYCLE_ID=PLAN-RESEARCH-1 \
  CAIRN_STORY_ID=STORY-RESEARCH-1 \
  CAIRN_SESSION_ID=SESSION-RESEARCH-1 \
...
planning_output="$(CAIRN_REQUIRED_BRANCH="$current_branch" CAIRN_DIRECTION_TEXT='Prioritize control plane hardening' CAIRN_DIRECTION_CONSTRAINTS='No human coding; low-vision-first' CAIRN_DIRECTION_NEXT_STAGE='architect' CAIRN_DIRECTION_CONFIRMED_BY='Matt' "$launch_script" planning)"
...
pm_output="$(CAIRN_REQUIRED_BRANCH="$current_branch" "$launch_script" pm)"
