...
path_doc="$root_dir/docs/operator/work-harness/process/PRE_CODING_PATH.md"
checklist="$root_dir/products/work-harness/operating-system/metrics/CODING_READINESS_GATE_CHECKLIST.md"
decision="$root_dir/products/work-harness/operating-system/decisions/CODING_READINESS_DECISION_2026-02-22.md"

assert_story_tracked() {
  local story="$1"
  local label="$2"
  if [[ -f "$root_dir/products/work-harness/delivery-backlog/engineering/active/$story" || -f "$root_dir/products/work-harness/delivery-backlog/engineering/qa/$story" || -f "$root_dir/products/work-harness/delivery-backlog/engineering/done/$story" ]]; then
    echo "PASS: $label"
  else
    echo "FAIL: $label"
