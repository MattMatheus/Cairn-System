...
validator="$root_dir/tools/platform/validate_tool_registry.sh"
...
CAIRN_TOOL_REGISTRY="$valid_registry" "$validator" >/dev/null
echo "PASS: validator accepts a valid approved registry"

set +e
invalid_output="$(CAIRN_TOOL_REGISTRY="$invalid_registry" "$validator" 2>&1)"
code=$?
set -e
