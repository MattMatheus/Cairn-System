...
## What Changed
- Added `tools/dev/bootstrap_platform.sh` to create the repo-local `.cairn/` runtime shape and verify the shared Go toolchain.
- Added `tools/dev/build_release_artifacts.sh` to build zipped `memory-cli` and `tool-cli` artifacts plus `SHA256SUMS` for `darwin/arm64` and `windows/amd64`.
- Added `azure-pipelines.yml` so Azure DevOps runs bootstrap, work harness doc tests, module-scoped Go tests, and publishes release artifacts.
- Fixed `products/work-harness/tools/check_go_toolchain.sh` and its docs/tests to use the real product module layout instead of a nonexistent root `go.mod`.
...
- Reused work harness doc tests for workflow/doc consistency.
