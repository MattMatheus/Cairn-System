# Story: Enrich tool-cli approved registry parameter metadata

## Metadata
- `id`: STORY-20260312-toolcli-registry-parameter-metadata
- `owner_persona`: Product Manager - work harness
- `status`: done
- `idea_id`: direct
- `phase`: v0.1
- `adr_refs`: [ADR-0005, ADR-0006, ADR-0007]
- `success_metric`: approved tool-cli tools with input parameters expose useful descriptions and enums in context output for operator-facing consumption
- `release_checkpoint`: deferred

## Problem Statement
- tool-cli context output now supports structured parameter metadata, but the approved registry still leaves most descriptions and enums empty, so the richer context is only partially useful.

## Scope
- In:
  - add parameter descriptions and enum metadata where appropriate in `products/tool-cli/registry/approved-tools.yaml`
  - update tests to verify the approved registry exposes richer parameter metadata
  - confirm `tool-cli context` surfaces the enriched metadata cleanly
- Out:
  - new tool entries
...
- `tool-cli context` structured parameter output
