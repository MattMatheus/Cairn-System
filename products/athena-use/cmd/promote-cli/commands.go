...
func runInspect(args []string) (err error) {
	...
	id := fs.String("id", "", "optional override for the AthenaMind entry id")
	title := fs.String("title", "", "optional override for the AthenaMind entry title")
	domain := fs.String("domain", "", "optional override for the AthenaMind entry domain")
...
func runNote(args []string) (err error) {
	...
	fs.SetOutput(os.Stderr)
	root := fs.String("root", defaultMemoryRoot(), "AthenaMind memory root")
	...
	tmp, err := os.CreateTemp("", "athena-promote-body-*.md")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
